...
agenda.define('requestProcess',{lockLifetime: 10000}, async(job, done) => {
    ...
    if(trip_detail.trip_status=='processing')
    {
        ...
        if(order_requests.length)
        {
            ...
            global.io.in("user_"+ order_requests[0].user_id).emit('trip_detail', { trip_detail, response_time });
        ...
        else
        {
            ...
            global.io.in("user_"+trip_detail.user_id).emit('trip_detail', { trip_detail, new_result });
            job.remove(err => {});
        }
    }
    else
